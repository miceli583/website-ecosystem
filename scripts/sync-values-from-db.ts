...
async function fetchCoreValues(): Promise<string[]> {
  ...
  return data
    .map((row: Value) => row.name || row.value || row.title || String(row.id))
    .filter(Boolean);
}
...
async function updateMarkdownFiles() {
  ...
  const existingCoreValues = fs
    .readFileSync(path.join(lifeOsDir, "CoreValues.md"), "utf-8")
    .split("\n")
    .filter((line) => line.trim());

  const existingSupportingValues = fs
    .readFileSync(path.join(lifeOsDir, "SupportingValues.md"), "utf-8")
    .split("\n")
    .filter((line) => line.trim());

  // Merge DB values with existing values (remove duplicates)
  const mergedCoreValues = Array.from(
    new Set([...coreValues, ...existingCoreValues])
  );
  const mergedSupportingValues = Array.from(
    new Set([...supportingValues, ...existingSupportingValues])
  );

  // Ensure all core values are in supporting values
  const finalSupportingValues = Array.from(
    new Set([...mergedCoreValues, ...mergedSupportingValues])
  );
  ...
  console.log(
    "\n✅ Updated CoreValues.md with",
    mergedCoreValues.length,
    "values"
  );
  console.log(
    "✅ Updated SupportingValues.md with",
    finalSupportingValues.length,
    "values"
  );
  console.log("\nCore Values includes:", mergedCoreValues.length, "values");
  console.log(
    "Supporting Values includes:",
    finalSupportingValues.length,
    "values (includes all Core Values)"
  );
}
