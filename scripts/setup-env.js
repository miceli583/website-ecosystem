...
function validateEnvironment() {
  ...
  if (missing.length > 0) {
    ...
    missing.forEach(({ key, config }) => {
      ...
      if ('setup' in config && config.setup) console.log(`   Setup: ${config.setup}`);
      if ('default' in config && config.default) console.log(`   Default: ${config.default}`);
      console.log();
    });
  }
...
function setupEnvironment() {
  ...
  console.log('\n📋 Next steps:');
  console.log('1. Configure database connection (SQLite default is fine for development)');
  console.log('2. Set up Supabase for production database and auth when needed');
